...
<html class="no-js" lang="en">
  ...
  <body>
   ...
   <!-- <div class="row" style="margin-top:10px;">
        ...
        <span class="input-group-cell"><span>Event Participation Report</span></span>
        </div>
        </div>
...
$sql="SELECT EventName,count(EventRegistration.EventID),AvailableSeats from Events INNER JOIN EventRegistration ON EventRegistration.EventID = Events.EventID 
Where Event_RegistrationStatus = 'Confirmed' 
group by EventRegistration.EventID;";
...
if(mysqli_num_rows($result)>0){
	while($row=mysqli_fetch_assoc($result)){
		echo "<tr style='font-size: 35px;'><center>";
			echo "<td><strong>". $row['EventName']."</strong></td>";
			echo "<td align='center'>". $row['count(EventRegistration.EventID)']."</td>";
			echo "<td align='center'>". $row['AvailableSeats']."</td>";
